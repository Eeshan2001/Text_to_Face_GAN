...
class ACGAN:
    def generateImage(txt):
        ...
        for i in inp:
            ...
            for c in i:
                if (c == ' '):
                    ...
                    k += c
            keys.append(k)
        print("Key before: ",keys)
        for i in range(len(keys) - 1, -1, -1):
            if keys[i] not in label:
                keys.pop(i)
        print("Key After: ", keys)
        if (keys == []):
        ...
        if (matches == []):
            ...
            choice =  str(choice)
            print("Before choice:",choice)
            while len(choice)<6:
                choice = '0'+choice
            print(" After Choice : ",choice)
            img = Image.open('C:\\Users\\Administrator\\Desktop\\Final-Year-Project-main\\dataset\\img_align_celeba\\' + choice + '.jpg')
            img = img.filter(ImageFilter.BoxBlur(1))
            img = img.resize((128, 128), Image.ANTIALIAS)
            img.save("C:/Users/Administrator/Desktop/Final-Year-Project-main/results/PROGAN_out.png", "PNG", quality=40)
            img = Image.open('C:\\Users\\Administrator\\Desktop\\Final-Year-Project-main\\dataset\\img_align_celeba\\' + choice + '.jpg')
            img = img.resize((64, 64), Image.ANTIALIAS)
            img = img.filter(ImageFilter.BoxBlur(1))
            img.save("C:\\Users\\Administrator\\Desktop\\Final-Year-Project-main\\results\\ACGAN_out.png", "PNG", quality=90)
